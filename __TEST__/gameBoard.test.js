...
describe("Place ship from GameBoard", () => {
    ...
    test("correct coordinates that the ship can be placed", () => {
        expect(board.getCoord(coordinates, aShip).right).toStrictEqual(
            receive.right
        );
        expect(board.getCoord(coordinates, aShip).left).toStrictEqual(
            receive.left
        );
        expect(board.getCoord(coordinates, aShip).up).toStrictEqual(receive.up);
        expect(board.getCoord(coordinates, aShip).down).toStrictEqual(
            receive.down
        );
    });
    test("ship register at board obj", () => {
        ...
        expect(board.placeShip(coordinates, aShip, true, "left")).toStrictEqual(
            receive.left
        );
    ...
    test("don't allow place ship in invalid board location", () => {
        let coord = "0,0";
        expect(board.placeShip(coord, aShip, true, "up")).toStrictEqual(
            "Not a valid move"
        );
        expect(board.placeShip(coord, aShip, true, "left")).toStrictEqual(
            "Not a valid move"
        );
    });
...
describe("board reaction after receive an attack", () => {
    ...
    test("ship get hit", () => {
        ...
        expect(board.receiveAttack("2,2")).toStrictEqual([
            "Submarine",
            2,
            false,
            [2, 2],
    ...
    test("ship is sunk after all part been hit", () => {
        expect(board.receiveAttack("2,3")).toStrictEqual([
            "Submarine",
            3,
            true,
            [2, 3],
